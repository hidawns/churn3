...
def show():
    ...
    df_cat = df.drop(columns=excluded_columns) if excluded_columns else df.copy()
    categorical_features_selected = df_cat.select_dtypes(include=['object']).columns.tolist()

    # Deteksi kolom churn
    churn_col = None
    for c in ['Churn Value', 'Churn']:
        if c in df.columns:
            churn_col = c
            break
    ...
    with st.expander("📊 **Univariate Analysis - Fitur Numerik**", expanded=True):
        if not numeric_features_selected:
            st.warning("Tidak ada fitur numerik ditemukan.")
        else:
    ...
    with st.expander("📊 **Univariate Analysis - Fitur Kategorikal**", expanded=False):
        if not categorical_features_selected:
            st.warning("Tidak ada fitur kategorikal ditemukan.")
        else:
    ...
    with st.expander("📊 **Heatmap**", expanded=False):
        ...
        if not numeric_cols:
            st.warning("Tidak ada variabel numerik untuk heatmap.")
        else:
